...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public void VerticalSrollToCell(int index, float speed)
        {
            if (totalCount >= 0 && (index < 0 || index >= totalCount))
            {
                Debug.LogWarningFormat("invalid index {0}", index);
            ...
            StopAllCoroutines();
            StartCoroutine(VerticalScrollToCellCoroutine(index, speed));
        }

        IEnumerator VerticalScrollToCellCoroutine(int index, float speed)
        {
            bool needMoving = true;
            while (needMoving)
            {
                ...
                float offset = 0.0f;
                // 위에 안보이는 셀 클릭
                if (m_ViewBounds.max.y < (m_ItemBounds.max.y + m_ContentSpacing))
                {
                    offset = (m_ViewBounds.max.y - m_ItemBounds.max.y);
                    offset -= m_ContentSpacing;
                    move = -Time.deltaTime * speed;
                }
                else if ((m_ItemBounds.min.y - m_ContentSpacing) < m_ViewBounds.min.y)
                {
                ...
                //Mathf.SmoothDamp(content.anchoredPosition.y, content.anchoredPosition.y + offset, ref speed, m_Elasticity, Mathf.Infinity, Time.unscaledDeltaTime);
                ...
                if (move != 0)
                {
                    ...
                    Vector2 toVectorOffset = GetVector(move);
                    content.anchoredPosition += toVectorOffset;
                    m_PrevPosition += toVectorOffset;
                    m_ContentStartPosition += toVectorOffset;
                }
            }
            StopMovement();
            UpdatePrevData();
        ...
        public void SetLastScroll()
        {
            SetContentAnchoredPosition(m_ViewBounds.max);
        }
        ...
        public Vector3 GetViewMaxBounds()
        {
            return m_ContentBounds.max;
        }
